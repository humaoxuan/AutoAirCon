...
class NEC():
    def __init__(self,
                 ...
                 leading_pulse_duration=9600,
                 # leading_gap_duration=4500,
                 leading_gap_duration=3500,
                 one_pulse_duration=1200,
                 # one_gap_duration=1686,
                 one_gap_duration=462,
                 zero_pulse_duration=400,
                 zero_gap_duration=462,
...
class IrSender():
    ...
    def send_code(self, ircode, nb=1):
        self.__log(LogLevel.Normal,
                   "Processing IR code: %s" % ' '.join([ircode[i:i + 8] for i in range(0, len(ircode), 8)]))
        for _ in range(0, nb):
...
# irs.send_code(bin(d).split('b')[1])
irs.send_code("1100011")