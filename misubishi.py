...
class Mitsubishi:
    ...
    def __send_command(self, climate_mode, temperature, fan_mode, vanne_vertical_mode, vanne_horizontal_mode, isee_mode,
                       area_mode, start_time, end_time, powerful, power_mode):
        ...
        self.__log(ir_sender.LogLevel.Verbose,
                   'TMP: {0:03d}  {0:02x}  {0:08b} (asked: {1})'.format(data[Index.Temperature], temperature))
        self.__log(ir_sender.LogLevel.Verbose, '')
        data[Index.ClimateAndHorizontalVanne] = ClimateMode.climate2(climate_mode) | vanne_horizontal_mode
        self.__log(ir_sender.LogLevel.Verbose,
                   'CLI: {0:03d}  {0:02x}  {0:08b}'.format(ClimateMode.climate2(climate_mode)))
        self.__log(ir_sender.LogLevel.Verbose, 'HOR: {0:03d}  {0:02x}  {0:08b}'.format(vanne_horizontal_mode))
        self.__log(ir_sender.LogLevel.Verbose,
                   'CLH: {0:03d}  {0:02x}  {0:08b}'.format(data[Index.ClimateAndHorizontalVanne]))
        self.__log(ir_sender.LogLevel.Verbose, '')
        ...
        self.__log(ir_sender.LogLevel.Verbose,
                   'ETI: {0:03d}  {0:02x}  {0:08b} {1}'.format(data[Index.EndTime], end_time))
        self.__log(ir_sender.LogLevel.Verbose, '')
        data[Index.StartTime] = 0 if start_time is None else ((start_time.hour * 6) + (start_time.minute // 10))
        self.__log(ir_sender.LogLevel.Verbose,
                   'STI: {0:03d}  {0:02x}  {0:08b} {1}'.format(data[Index.StartTime], start_time))
        self.__log(ir_sender.LogLevel.Verbose, '')
